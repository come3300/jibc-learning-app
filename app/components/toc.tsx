...
import * as tocbot from 'tocbot'; // 名前付きエクスポートを使用

function Toc() {
  useEffect(() => {
    // Tocbotの初期化
    tocbot.init({
      tocSelector: '.toc', // 目次の表示部分のクラス→つまりcssで左に記載しているスタイル名に目次を表示する
      contentSelector: '.post', // 目次を生成する対象のクラス→tocbot が .post 内の見出しを解析し、.toc 要素に目次を挿入します。
