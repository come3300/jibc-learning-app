...
{/* http://localhost:3000/documents/phase2/js-basic/third-step */}
<div class="bg-white">
    ...
    解説動画
    この動画教材ではテキストと同じようにDOMについて解説しています。
    テキストと動画の両方で学習をして理解を深めていきましょう。
    DOM講座
    ...
    * HTMLの特定の<button>タグがクリックされたときに確認アラートを表示する
    ...
    #### コンテンツツリー
    DOMでは、HTMLを親子関係のある「ツリー」とみなします。プログラミングの文脈でのツリーとは、
    ...
    const list = document.querySelector('.list')
    ```
    getElementByIdの他に、document.querySelectorでも要素を取り出すことができます。
    querySelectorでは、CSSのセレクターと同じ記法で要素を特定しとりだすことが可能です。
    ...
    document.createElementを使うと、HTML要素をJavaScript側で作ることができます。
    ...
    #### HTMLの準備
    以下ののindex.htmlを用意してください。この章を通してこのHTMLを利用します。
    ...
    ```
    また、main.jsを用意しておいてください。main.jsには、この後出てくるコード例を適宜記載していきます。
