...
macOSをお使いの方は、こちらのリンクからHomebrewをインストールするためのコードを取得します。
```
...
でインストールできます。インストールできたら、次のコマンドをコマンドラインで実行して正しくダウンロード出来ているか
確認してください。
```
$ git --version
```
...
では早速、Gitに触れていきます。
まずディレクトリを作成します。
今回はcodevillageディレクトリの中にintro-gitディレクトリを作成しましょう。
```
$ mkdir intro-git
```
...
それでは実際にコミットしてみます。コミットにはgit commit -m ""を使います。
...
ここでは次のコミットで何が記録されるかを表示してます。上のメッセージではindex.htmlが修正され、
...
```
これは2つのファイルが変更され、12行の追記が行われました。またmain.cssが作成されたことを表示しています。
リポジトリの変更を確認
ここまで2回のコミットをしてきました。次はこの2つのコミットを確認してみたいと思います。
```
$ git log
```
と実行して下さい。そうすると以下のようにコミットの記録が表示されます。
```
[master d36e835] 2回目のコミットです
 2 files changed, 12 insertions(+)
 create mode 100644 main.css
...
後はリモートリポジトリと紐づけてブランチを作成したらプッシュの準備完了なのでもう一息だけ頑張りましょう！
まずネット上にあるリモートリポジトリと手元にあるローカルリポジトリを紐づけます。以下のコマンドを実行してください。
```
git remote add origin https://github.com/(ユーザー名)/(リモートリポジトリ名).git
```

ブランチ名変更
英語の「ブランチ」とは「枝」などの意味がある。git のブランチは「コミット履歴の流れを分ける機能」です。
...
push
いよいよ push です。プッシュすることでリモートリポジトリにこれまでのコミットを書き込みます。
以下のコマンドを実行してください。
```
